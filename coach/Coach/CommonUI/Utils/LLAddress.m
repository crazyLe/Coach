    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            [self getAddressRequestWithParaDic:paraDic completeBlock:^(NSArray *addArr) {
                ...
                if (addArr.count>0) {
                    ...
                    if ([[kUserDefault objectForKey:cacheCityDataKey] boolValue]) {
                        [db_manager dropTableWithName:kOpenCityTableName];
                    }
                    
                    [db_manager createTableWithName:kOpenCityTableName keyArr:kOpenCityTableColumnArr];
                    [db_manager fastInsertIntoTable:kOpenCityTableName dicArr:addArr insertColumnArr:kOpenCityTableColumnArr];
                    
                    [kUserDefault setBool:YES forKey:cacheCityDataKey]; //记录已经缓存了市数据
                    [kUserDefault synchronize];
                    
                    /*
                    //从数据库中读取，验证
                    [db_manager getAllObjectsFromTable:kOpenCityTableName keyArr:kOpenCityTableColumnArr completionBlock:^(NSArray *array) {
                        if (array.count>0) {
                            //读到数据
                            [kUserDefault setBool:YES forKey:cacheCityDataKey]; //记录已经缓存了市数据
                        }
