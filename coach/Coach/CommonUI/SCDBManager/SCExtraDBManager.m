...
@implementation SCExtraDBManager
{
    FMDatabase *db;
    FMStatement *fm_state;
}

+ (SCExtraDBManager*)shareInstance;
{
    static SCExtraDBManager *manager;
    if (manager==nil)
    {
        manager = [[SCExtraDBManager alloc]init];
    }
    return manager;
}
...
//        {
//            //未更新过地址数据库
//            path = [[NSBundle mainBundle] pathForResource:@"DataBase" ofType:@"db"];
//        }
...
- (void)createTableWithName:(NSString *)tableName keyArr:(NSArray *)keyArr;
{
    /*
     ...
     [db close];
     */
    FMDatabaseQueue *queue = [SCExtraDBManager shareDatabaseQueue];
...
- (void)insertIntoTable:(NSString*)tableName dicArr:(NSArray *)dicArr insertColumnArr:(NSArray *)columnArr
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        for (NSDictionary *dic in dicArr) {
            ...
            [[SCExtraDBManager shareDatabaseQueue] inDatabase:^(FMDatabase *db) {
