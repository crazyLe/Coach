...
//    [self createNavWithLeftBtnImageName:@"返回" leftHighlightImageName:nil leftBtnSelector:@selector(leftClick) andCenterTitle:@"提现记录"];
    self.title = _type==WithdrawRecordControllerTypeRecharge?@"充值记录":@"提现记录";
}
-(void)leftClick
{
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    titleLbl.text = section==0?@"本月":((WithdrawGroupModel *)self.dataArray[section]).month;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.headBtn sd_setBackgroundImageWithURL:[NSURL URLWithString:self.currentUserInfo.face] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"iconfont-defaultheadimage"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        if (error) {
    ...
    if (model.pay_status==1) {
        ...
        cell.classLbl.text = [NSString stringWithFormat:@"已转账"];
    }
    else
    {
        cell.classLbl.text = [NSString stringWithFormat:@"未转账"];
    }
    return cell;
