...
-(NSArray *)queryCircleMessage
{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        NSString *sql =  @"select * from messageTable where msg_id=29 ORDER BY id DESC ";
        FMResultSet *resultSet =  [db executeQuery:sql];
        // 遍历结果集合
        while ([resultSet next])
        {
...
-(NSArray *)queryCircleUnRead
{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        NSString *sql =  @"select * from messageTable where msg_id=29 and isRead=0 ORDER BY id DESC ";
        FMResultSet *resultSet =  [db executeQuery:sql];
        // 遍历结果集合
        while ([resultSet next])
        {
...
- (void)deleteMessgateWithId:(NSString *)idStr
{
    [queue inDatabase:^(FMDatabase *db) {
       
        NSString *sql = [NSString stringWithFormat:@"delete from messageTable where id = %@",idStr];
