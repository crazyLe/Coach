...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        //置顶
        self.topBestImageView = [UIButton buttonWithType:UIButtonTypeCustom];
        [self.topBestImageView setBackgroundImage:[UIImage imageNamed:@"矩形zhiding-2"] forState:UIControlStateNormal];
        [self.topBestImageView setTitle:@"置顶" forState:UIControlStateNormal];
        self.topBestImageView.titleLabel.font = Font12;
        [self.contentView addSubview:self.topBestImageView];
        ...
        [self.zanBtn setImage:[UIImage imageNamed:@"圈子_iconfont-circle-zan-拷贝"] forState:UIControlStateSelected];
        ...
        [self.commentZanBtn setImage:[UIImage imageNamed:@"圈子_iconfont-circle-zan-拷贝"] forState:UIControlStateSelected];
...
-(void)setupFrame
{
    ...
    self.topBestImageView.frame = _circleFrameModel.topBestImageViewF;
...
-(void)setupData
{
    ...
    {
        self.topBestImageView.hidden = NO;
    }
    else
    {
        self.topBestImageView.hidden = YES;
    }
    ...
    [self.iconImageView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"iconfont-defaultheadimage"]  completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        if (error) {
            self.iconImageView.image = [UIImage imageNamed:@"iconfont-defaultheadimage"];
        }
    }];
    ...
    for (int i = 0; i<self.imageViewsArray.count; i++) {
        ...
        [imageView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"iconfont-default-1-1-image"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            if (error) {
                imageView.image = [UIImage imageNamed:@"iconfont-defaultloadfailed"];
            }
        }];
    }
    ...
    if(self.circleFrameModel.circleMainModel.comemnt.addtime != nil)//有评论
    {
        ...
        [self.commentIconImageView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"iconfont-defaultheadimage"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            if (error) {
                self.commentIconImageView.image = [UIImage imageNamed:@"iconfont-defaultheadimage"];
            }
        }];
...
//        self.commentZanBtn.selected = 
    }
    
    self.zanBtn.selected = self.circleFrameModel.circleMainModel.is_praise;
}
